...
<head>
    ...
    <style>
        ...
        /* Smooth row hover */
        .table-row-hover:hover { background: #caf0f8; }
        /* Animations */
        @keyframes slideDown {
        ...
        .sidebar::-webkit-scrollbar-thumb { background: #e8eff5; border-radius: 4px; }
    </style>
</head>
...
<aside class="sidebar fixed top-0 left-0 w-[260px] h-screen bg-white border-r border-border z-[1000] flex flex-col overflow-y-auto max-lg:-translate-x-full" id="sidebar">
    <div class="px-5 py-6 flex items-center gap-3.5 border-b border-border">
        <div class="w-11 h-11 bg-gradient-to-br from-ocean-dark via-ocean-mid to-ocean-light rounded-[14px] flex items-center justify-center shrink-0 shadow-[0_4px_12px_rgba(0,119,182,0.25)]">
            <i class="fas fa-users-cog text-white text-lg"></i>
        </div>
        <div>
            <h5 class="text-text-primary font-extrabold text-lg tracking-tight leading-tight">SiManPeg</h5>
    ...
    <nav class="px-3.5 py-4 flex-1">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
            <i class="fas fa-chart-pie w-5 text-center text-[15px]"></i>
            Dashboard
        </a>
        <a href="{{ route('pegawai.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('pegawai.*') && !request()->routeIs('pegawai.import*') && !request()->routeIs('pegawai.export*') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
        ...
        <a href="{{ route('pegawai.import.form') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('pegawai.import*') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
            <i class="fas fa-file-import w-5 text-center text-[15px]"></i>
            Import Data
        </a>
        <a href="{{ route('pegawai.export.preview') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('pegawai.export*') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
        ...
        <a href="{{ route('audit.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('audit.*') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
            <i class="fas fa-history w-5 text-center text-[15px]"></i>
            Riwayat Aktivitas
        </a>
        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium mb-1 transition-all duration-200 {{ request()->routeIs('profile.*') ? 'bg-gradient-to-r from-ocean-dark to-ocean-mid text-white shadow-[0_4px_12px_rgba(0,119,182,0.25)]' : 'text-text-secondary hover:bg-ocean-wash hover:text-ocean-dark' }}">
    ...
    <div class="px-5 py-4 border-t border-border bg-bg">
        <div class="flex items-center gap-3 mb-3.5">
            <div class="w-[38px] h-[38px] bg-gradient-to-br from-ocean-dark to-ocean-light rounded-[10px] flex items-center justify-center text-sm text-white font-bold">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full py-2.5 rounded-[10px] border-[1.5px] border-border bg-white text-text-secondary text-[13px] font-medium cursor-pointer transition-all duration-200 flex items-center justify-center gap-2 hover:bg-red-50 hover:border-red-200 hover:text-red-600">
...
<main class="ml-[260px] min-h-screen max-lg:ml-0 transition-[margin-left] duration-300">
    <div class="px-8 pt-6 pb-2 flex items-center justify-between max-sm:px-4">
        <div>
            ...
            <h4 class="text-2xl font-bold text-text-primary tracking-tight">@yield('page-title', 'Dashboard')</h4>
    ...
    <div class="px-8 py-5 pb-8 max-sm:px-4">
        @if(session('success'))
            <div class="flex items-center gap-2.5 px-5 py-3.5 rounded-[10px] bg-emerald-50 text-emerald-600 border border-emerald-200 text-sm mb-3 animate-slideDown">
